...
//Double jump
    if(vAniID == openborconstant("ANI_JUMP") || vAniID == openborconstant("ANI_FOLLOW1") ){ //Jumping or bounced after diving?
...
//Dive kick
    if(vAniID == openborconstant("ANI_FREESPECIAL") ){ //On double jump?
      if(iJump){ //Jump pressed?
        if(iLeftH){ //Left held?
          changeentityproperty(vSelf, "direction", 0);
